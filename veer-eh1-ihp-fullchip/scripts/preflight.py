...
bridge = (ROOT / "config/veer_asic/veer_config.sv").read_text(encoding="utf-8")
if '`include "common_defines.vh"' not in bridge:
    errors.append("veer_config.sv does not include common_defines.vh")
if not re.search(r"SLANG_ARGUMENTS:\s*\[[^\]]*--single-unit", text):
    errors.append("SLANG_ARGUMENTS lacks --single-unit")
if "dir::../config/veer_asic/veer_config.sv" not in text:
    errors.append("veer_config.sv is not the first configuration source")
